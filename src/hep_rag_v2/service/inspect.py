    ...
    *,
    ...
    with connect() as conn:
        ...
        neighbors = graph_neighbors(
            ...
            limit=limit,
            similarity_model=similarity_model if edge_kind == "similarity" else None,
        )
    return {
        "work": _work_identity_payload(work),
        "edge_kind": edge_kind,
        "collection": collection,
        "neighbors": neighbors,
    }
...
def show_document_payload(
    *,
    ...
    with connect() as conn:
        ...
        if document is None:
            raise ValueError(f"No document materialized for work_id={int(work['work_id'])}")
        ...
        sections = [
            ...
            for row in conn.execute(
                ...
                LIMIT ?
                """,
                (int(document["document_id"]), limit),
            )
        ]
        blocks = [
            dict(row)
...
def audit_document_payload(
    *,
    ...
    with connect() as conn:
        ...
        payload = _audit_document_payload(conn, document_id=int(document["document_id"]), limit=limit)
        payload["document"] = dict(document)
        payload["work"] = _work_identity_payload(work)
    return payload
...
def _audit_document_payload(conn: sqlite3.Connection, *, document_id: int, limit: int) -> dict[str, Any]:
    ...
    search_counts = _safe_search_counts(conn, document_id=document_id)
...
def _safe_search_counts(conn: sqlite3.Connection, *, document_id: int) -> dict[str, int]:
    try:
        row = conn.execute(
            """
            SELECT
...
def _work_identity_payload(work: sqlite3.Row | dict[str, Any]) -> dict[str, Any]:
    return {
        "work_id": int(work["work_id"]),
